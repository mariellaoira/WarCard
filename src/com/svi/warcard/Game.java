...
public class Game {
	...
	public Game(int players, int shuffels) {
		...
		/**
		 * Array lists that will rank 'Suits' and 'Ranks' depending on index. 0 is the
		 * highest.
		 */
	...
	/**
	 * Method that gets the winning player and Card bases on the cards put on the
	 * table.
	 * 
	...
	/**
	 * Method that adds rounds. The loop will not stop until the start index matches
	 * the cards' size.
	 * 
	...
	/**
	 * For bug fixes only. Sometimes, the program has inaccurate rounds because of
	 * the players' cards in hand.
	 * 
	...
	public static void main(String[] args) throws InterruptedException {
		...
		} while (numOfPlayers < 2 || numOfPlayers > 52);
		...
		Game game = new Game(numOfPlayers, shuffles);
		...
		System.out.println("Deck after shuffling for " + shuffles + " time(s)");
		System.out.println(game.deck + "\n");
		...
		while (!game.gameOver()) {
			//This section will create a new round, get the top card of each player. Then, will be transferred to the 'cardsToCompare' list.
			...
			for (Player player : game.players) {
				...
				// Displays: Player revealing their top cards along with remaining cards. If
				// players has no cards, display 'Nothing'
				System.out.println("Player : " + player.getId() + " reveals: " + (card == null ? "Nothing, " : card)
						+ " Remaining Cards Size:" + player.getCardsInHand().size() + " Remaining Cards: " + player);
				index++;
			}
			//This section will show the winning card. It will depend on the cards' hierarchy. Lowest index is the winner.
